...
with st.sidebar:
    ...
    preset_sig = (tuple(all_leagues), tuple(sorted(sel_regions)), use_top5, use_top20, use_efl,
    ...
    # drop any stored leagues that don't exist in the current CSV / region filter
    _stored = [lg for lg in st.session_state.get("ts_leagues_sel", default_leagues) if lg in region_leagues]
    st.session_state["ts_leagues_sel"] = _stored if _stored else default_leagues
...
if _tri_img:
    st.image(_tri_img, width="stretch")
...
# ══════════════════════════════════════════════════════
# SECTION 5B – TEAM ONE-PAGER
# ══════════════════════════════════════════════════════
st.subheader("📄 Team One-Pager")

_op_team  = sel_team
_op_tdf   = df[df["Team"] == _op_team]

if _op_tdf.empty:
    st.info("Team not found.")
else:
    _op_r      = _op_tdf.iloc[0]
    ...
    _p_pass  = _op_pct("Passes p90")
    _p_xga   = _op_pct("xG Against p90", invert=True)
    ...
    _pts_pos,  _pts_n  = get_league_pos(_op_r, df, "Points",          ascending=False)
    _xpts_pos, _xpts_n = get_league_pos(_op_r, df, "Expected Points", ascending=False)
    _pts_str  = f"{_pts_pos}/{_pts_n}"   if _pts_pos  is not None else "—"
    _xpts_str = f"{_xpts_pos}/{_xpts_n}" if _xpts_pos is not None else "—"

    _op_ovr = float(_op_r.get("OVR", 0) or 0)
    _op_att = float(_op_r.get("ATT", 0) or 0)
    ...
    def _opval(col):
        ...
        if pd.isna(v): return "—"
        fv = float(v)
        if "%" in col: return f"{int(round(fv))}%"
        return f"{fv:.2f}"

    def _triples(pairs):
        return [(lbl, _op_pct(col, col in _INV_COLS), _opval(col))
                for lbl, col in pairs if col in df.columns]
    ...
    _OP_POS = _triples([
        ...
        ("Possession",       "Possession %"),
        ("Passes",           "Passes p90"),
        ("Pass Acc %",       "Pass Accuracy %"),
        ("Long Passes",      "Long Passes p90"),
        ("Long Pass %",      "Long Pass Accuracy %"),
        ("Passes Final 3rd", "Passes to Final Third p90"),
        ("Prog Passes",      "Progressive Passes p90"),
    ...
    def _pool_rate_pct(col, div_col):
        if col not in _op_pool.columns or div_col not in _op_pool.columns: return pd.Series(dtype=float)
        r = pd.to_numeric(_op_pool[col],     errors="coerce")
        d = pd.to_numeric(_op_pool[div_col], errors="coerce").replace(0, np.nan)
        return (r/d).dropna()
    ...
    BAR_PX = 24; GAP_PX = 6; SEP_PX = 2; STEP_PX = BAR_PX + GAP_PX
    LABEL_FS = 10.6; VALUE_FS = 8.5; TITLE_FS = 20

    def _dc(v):
        if pd.isna(v): return (0.6, 0.63, 0.66)
        v = float(v)
        if v<=50: t=v/50.0;       c1,c2=np.array([239,68,68]),  np.array([234,179,8])
        else:     t=(v-50)/50.0;  c1,c2=np.array([234,179,8]),  np.array([34,197,94])
    ...
    def _rc(v):
        ...
        if v>=66: return "#7FBC41","#000"
        if v>=54: return "#A7D763","#000"
        if v>=44: return "#F6D645","#000"
    ...
    def _tw(fig, s, *, fontsize=8, weight="normal"):
        ...
        except: r=getattr(fig.canvas,"renderer",None)
        if r is None: t.remove(); return len(str(s))*fontsize*0.55/fig.bbox.width
        w=t.get_window_extent(renderer=r).width; t.remove()
        return w/fig.bbox.width

    def _th(fig, s="Hg", *, fontsize=8, weight="normal"):
        t=fig.text(0,0,s,fontsize=fontsize,fontweight=weight,
                   transform=fig.transFigure,alpha=0)
        fig.canvas.draw()
        try:    r=fig.canvas.get_renderer()
        except: r=getattr(fig.canvas,"renderer",None)
        if r is None: t.remove(); return fontsize*1.4/fig.bbox.height
    ...
    def _bar_panel(fig, left, top, width, triples, title, forced_gutter=None):
        ...
        bx=left+gutter; bw2=max(0.001,width-gutter-0.004)
        ax=fig.add_axes([bx,bottom,bw2,ax_h])
        ax.set_facecolor(PANEL_BG)

        pcts=[float(np.nan_to_num(t[1],nan=0.0)) for t in triples]
        texts=[t[2] for t in triples]
        bar_du=BAR_PX/STEP_PX; gap_du=GAP_PX/STEP_PX
        track_h=bar_du+gap_du-SEP_PX/STEP_PX
        ...
        for yi,lab in zip(y_idx[:n],labels):
            ...
            fig.text(left+0.003,yf,lab,color=FTXT,
                     fontsize=LABEL_FS,fontweight="bold",va="center",ha="left")
        fig.text(left+0.003,bottom+ax_h+0.008,title,
                 color=FTXT,fontsize=TITLE_FS,fontweight="900",ha="left",va="bottom")
...
if lb_metric not in df.columns:
    ...
    st.pyplot(fig_lb, width="stretch")
    _buf_lb = io.BytesIO()
...
if _sc_df.empty:
    ...
    try:
        ...
        if sc_render_exact:
            ...
            st.pyplot(fig_sc, width="content")
...
with st.expander("Similar Teams settings", expanded=False):
    ...
    st.markdown("#### League Presets")
    _sp = st.columns(3)
    _sim_top5  = _sp[0].checkbox("Top 5",  False, key="ts_sim_top5")
    _sim_top20 = _sp[1].checkbox("Top 20", False, key="ts_sim_top20")
    _sim_efl   = _sp[2].checkbox("EFL",    False, key="ts_sim_efl")

    st.markdown("#### GBE Bands")
    _sbc1 = st.columns(3)
    _sim_b1 = _sbc1[0].checkbox("Band 1", False, key="ts_sim_band1")
    _sim_b2 = _sbc1[1].checkbox("Band 2", False, key="ts_sim_band2")
    _sim_b3 = _sbc1[2].checkbox("Band 3", False, key="ts_sim_band3")
    _sbc2 = st.columns(3)
    _sim_b4 = _sbc2[0].checkbox("Band 4", False, key="ts_sim_band4")
    _sim_b5 = _sbc2[1].checkbox("Band 5", False, key="ts_sim_band5")
    _sim_b6 = _sbc2[2].checkbox("Band 6", False, key="ts_sim_band6")

    st.markdown("#### League Strength")
    _sim_use_strength = st.toggle("Filter by league strength", False, key="ts_sim_use_strength")
    if _sim_use_strength:
        _sim_strength_range = st.slider("Strength range (0–100)", 0, 100, (50, 100), key="ts_sim_strength_range")
    else:
        _sim_strength_range = (0, 100)

    _sim_opts = list(_st_all_leagues)
    if _sim_use_strength:
        _sim_opts = [lg for lg in _sim_opts
                     if _sim_strength_range[0] <= LEAGUE_STRENGTHS.get(str(lg).strip(), 50.0) <= _sim_strength_range[1]]

    _sim_seed = set()
    if _sim_top5:  _sim_seed |= PRESET_LEAGUES["Top 5 Europe"]
    if _sim_top20: _sim_seed |= PRESET_LEAGUES["Top 20 Europe"]
    if _sim_efl:   _sim_seed |= PRESET_LEAGUES["EFL (England 2–4)"]
    _sim_bands = {b for b, f in [(1,_sim_b1),(2,_sim_b2),(3,_sim_b3),(4,_sim_b4),(5,_sim_b5),(6,_sim_b6)] if f}
    if _sim_bands:
        _sim_seed |= {lg for lg in _sim_opts if gbe_league_band(lg) in _sim_bands}
    _sim_seed = {lg for lg in _sim_seed if lg in _sim_opts}

    _sim_any_preset = bool(_sim_top5 or _sim_top20 or _sim_efl or _sim_bands or _sim_use_strength)

    if _sim_seed:
        _st_league_default = sorted(_sim_seed)
    elif _sim_any_preset:
        _st_league_default = _sim_opts
    else:
        # Default pool = whatever the sidebar leagues filter is currently set to
        _st_league_default = [lg for lg in leagues_sel if lg in _sim_opts] or (
            [team_league] if team_league in _sim_opts else _sim_opts[:1]
        )

    _sim_key = f"ts_sim_leagues_{sel_team}"
    _sim_sig = (tuple(_st_all_leagues), _sim_top5, _sim_top20, _sim_efl,
                _sim_b1, _sim_b2, _sim_b3, _sim_b4, _sim_b5, _sim_b6,
                _sim_use_strength, _sim_strength_range)
    if st.session_state.get(f"ts_sim_sig_{sel_team}") != _sim_sig:
        st.session_state[f"ts_sim_sig_{sel_team}"] = _sim_sig
        st.session_state[_sim_key] = _st_league_default

    _sim_stored = [lg for lg in st.session_state.get(_sim_key, _st_league_default) if lg in _sim_opts]
    st.session_state[_sim_key] = _sim_stored if _sim_stored else _st_league_default

    st_leagues = st.multiselect(
        "Candidate league pool", _sim_opts,
        key=_sim_key,
    )
    st_incl_target = st.toggle(
        "Include target team even if outside the preset", value=True, key="ts_sim_incl_target",
        help="Keeps the target team's own league in the comparison pool so its percentiles can be calculated. "
             "It stays out of the results list."
    )
...
if _sim_target_rows.empty or not SIM_TEAM_COLS:
    ...
    try:
        ...
        _st_leagues_full = (
            list(set(list(st_leagues) + ([_sim_tgt_league] if st_incl_target else [])))
            if st_leagues else None
        )
        ...
        if _st_pool_all.empty:
        ...
        elif not (_st_pool_all["Team"] == sel_team).any():
            st.info(
                f"**{sel_team}** ({_sim_tgt_league}) is outside the selected league pool — "
                "turn on *Include target team even if outside the preset* in settings."
            )
        else:
            ...
            if _cand_df.empty:
                ...
                if _stri_img:
                    st.image(_stri_img, width="stretch")
                    st.download_button(
